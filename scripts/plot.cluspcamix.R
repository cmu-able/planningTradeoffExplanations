plot.cluspcamix<-function(x, dims = c(1,2), cludesc = FALSE, topstdres = 20, objlabs = FALSE, attlabs = NULL, attcatlabs = NULL, subplot = FALSE, what = c(TRUE,TRUE), max.overlaps = 10, cluster_names = NULL, dimensions = NULL){
  d1 = NULL
  d2 = NULL
  gr = NULL
  olab = NULL
  ...
  if (is.null(cluster_names))
    cluster_names <- c(paste("C",1:K,sep=""))
 
  if (is.null(attlabs)) {
    attlabs=rownames(x$attcoord)
  }
  
  if (is.null(attcatlabs)) {
    attcatlabs=rownames(x$attcatcoord)
  } else {
  ...
  if(what[1]==TRUE && what[2]==FALSE ){
    a=ggplot(data=obs_df,aes(x=d1,y=d2))
    a=a+geom_point(aes(x=d1,y=d2,colour=gr,shape=gr,alpha=.4),size=1)+theme_Publication()
    if (objlabs == TRUE) {
    ...
    if (!is.null(dimensions))
      a=a+xlab(paste("Dim. ",dims[1], " (",dimensions[1],"%)", sep = "")) + ylab(paste("Dim. ",dims[2],  " (",dimensions[2],"%)", sep = ""))
    else
  ...
  if(what[1]==FALSE && what[2]==TRUE ){
    ...
    a=ggplot(data=obs_df,aes(x=d1,y=d2))+theme_Publication()
    ...
    if (!is.null(dimensions))
      attquali_plot=attquali_plot+xlab(paste("Dim. ",dims[1], " (",dimensions[1],"%)", sep = "")) + ylab(paste("Dim. ",dims[2],  " (",dimensions[2],"%)", sep = ""))
    else
      attquali_plot=attquali_plot+xlab(paste("Dim. ",dims[1])) + ylab(paste("Dim. ",dims[2]))
    if (length(x$size) != 1)
    {
  ...
  if(what[1]==TRUE && what[2]==TRUE ){
    a=ggplot(data=obs_df,aes(x=d1,y=d2))
    a=a+geom_point(mapping=aes(x=d1,y=d2,shape=gr,alpha=.4),inherit.aes = FALSE,alpha=0.5,size=1)+theme_Publication()
    if (objlabs == TRUE) {
      a=a+geom_text_repel(data=obs_df,aes(label=olab), max.overlaps = max.overlaps)
    }
    ...
    attquali_plot = att_catdf %>% ggplot(aes(x = d1, y = d2))+
      ...
      xlab(paste("Dim. ",dims[1], " (",dimensions[1],"%)", sep = "")) + ylab(paste("Dim. ",dims[2],  " (",dimensions[2],"%)", sep = "")) +
      theme_Publication() +
      theme(legend.title=element_blank(),legend.position="none",axis.text.x = element_blank(),axis.text.y = element_blank())
  ...
  if(cludesc==TRUE){
    ...
    if (anynum) {
      ...
      X = QuantiAct
      cdsc = clu_means(X, x$cluster, center=FALSE, scale=FALSE, cluster_names=cluster_names)
      out$parcoord = cdsc
    }
    ...
    if (anyfact){
      ...
      if (subplot == TRUE) 
        TopplotGroups=outOfIndependence(data[, facAct, drop = FALSE],Gvec=x$cluster,attlabs,firstfew=ffew,textSize=3.5,segSize=4,myleftmarg=5,myrightmarg=1, cluster_names=cnm)
      else
        TopplotGroups=outOfIndependence(data[, facAct, drop = FALSE],Gvec=x$cluster,attlabs,firstfew=ffew,textSize=3.5,segSize=4,myleftmarg=0.5, myrightmarg=0.5, cluster_names=cnm)
      
      plotGroups=outOfIndependence(data[, facAct, drop = FALSE],Gvec=x$cluster,nolabs=TRUE,attlabs,fixmarg=FALSE,textSize=1.5,segSize=1.5, cluster_names=cnm)
      
      for(jjj in 1:K){
        TopplotGroups$G[[jjj]]=TopplotGroups$G[[jjj]]+theme_Publication()+ggtitle(cnm[[jjj]])
